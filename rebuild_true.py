...
def penalty_categorical(y_true,y_pred):
    boxel_kid = K.sum(K.cast((K.equal(K.argmax(y_true,axis=-1),1)),"float32"))
    boxel_can = K.sum(K.cast((K.equal(K.argmax(y_true,axis=-1),2)),"float32"))
    
    
    
    boxel_kid = K.log(boxel_kid)
    boxel_can = K.log(boxel_can)
    #boxel_kid = np.cbrt(K.eval(boxel_kid))
    #boxel_can = np.cbrt(K.eval(boxel_can))
    ...
    if boxel_kid==0:
        ...
        y_pred_1 = y_pred[:,:,:,:,1] / boxel_kid 
    
    if boxel_can==0:
        y_pred_2 = y_pred[:,:,:,:,2] 
    else:
        y_pred_2 = y_pred[:,:,:,:,2] / boxel_can
...
def main(_):
    ...
    with tf.device('/device:GPU:{}'.format(args.gpuid)):
        ...
        model.compile(loss=penalty_categorical, optimizer=optimizer, metrics=[dice])
